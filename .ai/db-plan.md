...
**Wymagane rozszerzenia**: `pgcrypto` (UUID: `gen_random_uuid()`), `citext` (unikalność case-insensitive), **opcjonalnie
** `pg_trgm` (wyszukiwanie po nazwie zestawu).
...
- `current_app_user()` → `UUID`
  ...
  $$;
  ```
- **Uwaga**: Aplikacja (Symfony/Doctrine) musi ustawiać `SET app.current_user_id = '<uuid>';` na początku
  żądania/transakcji.
...
- **Soft delete**: wszystkie listowania w aplikacji powinny filtrować `deleted_at IS NULL`. Polityki RLS również
  filtrują wiersze miękko usunięte.
...
- **Ewolucja**: przy wprowadzeniu współdzielenia zestawów w przyszłości — rozszerzyć model o ACL/role i zaktualizować
  polityki RLS.
